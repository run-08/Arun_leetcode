...
//         int n = grid.length,min=100000;
//         dp = new int[n][n];
//         for(int i=0;i<n;i++) for(int j=0;j<n;j++) Arrays.fill(dp[i],-1);
//         for(int i=0;i<n;i++) min = Math.min(min,grid[0][i]+helper(grid,i,1,n));
//         return min;
//     }
//     private int helper(int[][] grid,int previous,int row,int n){
//         if(row >= n) return 0;
//         if(dp[row][previous] != -1) return dp[row][previous];
//         int min = 100000;
//         for(int i=0;i<n;i++){
//             if(i==previous) continue;
//             min = Math.min(min,grid[row][i]+helper(grid,i,row+1,n));
//         }
//         return dp[row][previous]=min;
//     }
// }
